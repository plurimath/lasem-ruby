...
`lasem` is a native extension. Ruby 3.2 or newer is required, and the upstream
...
`input`::
  ...
  Default `:xml`. `:latex` and `:itex` are passed unchanged to Lasem's itex
  parser; the gem does not infer or add math delimiters.
...
`offset_x`, `offset_y`::
  Optional translation in user units. Default `0.0`.
...
. A source-checkout vendored Lasem install under `vendor/lasem/install`.
...
It reports missing executables, missing pkg-config packages, and submodule
state. Output ends with `Required dependencies look available.` once the
